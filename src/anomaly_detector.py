...
class AnomalyDetector:
    ...
    def score_wallet_suspiciousness(
        ...
        # Suspicious timing - off-hours trading
        trade_timestamp = trade.get("timestamp", 0)
        if trade_timestamp > 0:
            trade_time = datetime.fromtimestamp(trade_timestamp)
            hour = trade_time.hour
            # Check if trade happened during unusual hours (2 AM - 5 AM UTC)
            if 2 <= hour <= 5:
                score += 1
                reasons.append(f"Off-hours trading: {trade_time.strftime('%H:%M UTC')}")
    ...
    def detect_volume_spike(self, trades: list[dict], recent_trades: list[dict]) -> dict:
        """
        Detect if recent trading volume is significantly higher than historical average
        This could indicate insider knowledge before news breaks
        """
        if not trades or not recent_trades:
            return {"has_spike": False}

        # Calculate historical average (last 24h)
        historical_sizes = [
            float(t.get("size", 0)) * float(t.get("price", 0)) for t in trades
        ]
        historical_avg = statistics.mean(historical_sizes) if historical_sizes else 0

        # Calculate recent volume (last hour approximation)
        if len(recent_trades) > 0:
            recent_sizes = [
                float(t.get("size", 0)) * float(t.get("price", 0)) for t in recent_trades
            ]
            recent_avg = statistics.mean(recent_sizes) if recent_sizes else 0

            # Check if recent volume is significantly higher (3x or more)
            if historical_avg > 0 and recent_avg > historical_avg * 3:
                return {
                    "has_spike": True,
                    "multiplier": recent_avg / historical_avg,
    ...
    def detect_price_momentum(self, trades: list[dict]) -> dict:
        """
        Detect if there's unusual price momentum that might indicate insider trading
        """
        ...
        prices = [float(t.get("price", 0)) for t in recent_trades if float(t.get("price", 0)) > 0]
        ...
        # If 80%+ of moves are in same direction, flag it
        total_moves = len(price_changes)
        if total_moves > 0:
            direction_ratio = max(positive_moves, negative_moves) / total_moves
            if direction_ratio >= 0.8:
                return {
                    "has_momentum": True,
                    "direction": "bullish" if positive_moves > negative_moves else "bearish",
                    "consistency": direction_ratio,
